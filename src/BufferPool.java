...
public class BufferPool {
	 ...
	 * 
	 * @param file the file that stores the data being read in
	 * @param num  number of buffer arrays to be created
	 * @param stat stat object so track efficiency metrics
	 * @throws FileNotFoundException throw if the file does not exist in the library
	 */
	 ...
	 * 
	 * @param space the byte array which will hold the 4 byte record
	 * @param sz    size of the record being read, here assumed to be 4 as the data
	 *              in the file is 4 byte records
	 * @param pos   position of the beginning of the 4 byte to be read
	 * @throws IOException throws upon failure to acquire or write i/o sufficiently
	 */
	public void getbytes(byte[] space, int sz, int pos) throws IOException {
		...
		for (int i = 0; i < size; i++) {
			if (buffers[i] != null && buffers[i].getBlockIndex() == block) {
				...
				buffers[0] = temp;
				stat.increaseHits();
				System.arraycopy(temp.getData(), start, space, 0, sz);
				return;
			}
		}
		/*
	 ...
	 * 
	 * @param space the byte array which will hold the 4 byte record
	 * @param sz    size of the record being inserted, here assumed to be 4 as the
	 *              data in the file is 4 byte records
	 * @param pos   position of the beginning of the 4 byte to be edited
	 * @throws IOException throws upon failure to acquire or write i/o sufficiently
	 */
	public void insert(byte[] space, int sz, int pos) throws IOException {
		...
		for (int i = 0; i < size; i++) {
			if (buffers[i] != null && buffers[i].getBlockIndex() == block) {
				...
				buffers[0] = temp;
				stat.increaseHits();
				System.arraycopy(space, 0, temp.getData(), start, sz);
				temp.setDirty(true);
				return;
			}
		}
