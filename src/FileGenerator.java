...
/**
 * Generate a test data file of records. Each record is 4 bytes: 2 bytes for the
 * key (a java short, used for sorting), 2 bytes for the value (a java short). A
 * group of 2048 records is a block. Depending on the method, you can generate
 * two types of files: ASCII or raw binary shorts. In ASCII mode, the records
 * are constrained to specific values which align with specific ASCII values.
 * Reading this file as text/ascii will show a record in this format:
...
public class FileGenerator {
	 ...
	 * Important constants
	 */
	static public final int BYTES_PER_BLOCK = RECORDS_PER_BLOCK * BYTES_PER_RECORD;
	 ...
	 * random. Files generated using the same seed will be exactly the same. Can be
	 * helpful for consistent testing.
	 * 
	 * @param seed the seed
	 */
	...
	/**
	 * Generates a file of random ASCII records. This is just to make it easier for
	 * your visual inspection Record keys are one space ' ' and a randomly ASCII
	 * charachter within in the range ' A' to ' Z' . Record values are always two
	 * spaces' '.
	 * 
	...
	/**
	 * Generates a file of random binary records. Record keys and values are shorts
	 * in range [1-30000)
	 * 
	 * @param dos The data output stream to write data to
	 * @throws IOException if writing shorts encounters an issue
